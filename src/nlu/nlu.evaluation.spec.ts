...
describe('NLU AI Engine 100-Case Evaluation Benchmark', () => {
  ...
  beforeAll(() => {
    EVALUATION_DATASET.forEach((testCase) => {
      ...
      if (
        ...
        category.toLowerCase().includes(testCase.expectedCategory.toLowerCase())
      ) {
        passedCategory++;
      }
      ...
      // Check Bill Splits
      if (
        !testCase.expectedSplitCount ||
        parsed.splitCount === testCase.expectedSplitCount
      ) {
        passedSplits++;
      }
    });
  ...
  it(`should achieve >= 95% Amount Extraction Accuracy across ${totalCases} test cases`, () => {
    ...
    console.log(
      `📊 Amount Extraction Accuracy: ${amountAccuracy.toFixed(1)}% (${passedAmount}/${totalCases})`,
    );
  ...
  it(`should achieve >= 95% Type (Income/Expense) Classification Accuracy`, () => {
    ...
    console.log(
      `📊 Type Classification Accuracy: ${typeAccuracy.toFixed(1)}% (${passedType}/${totalCases})`,
    );
  ...
  it(`should achieve >= 90% Category Mapping Accuracy`, () => {
    ...
    console.log(
      `📊 Category Mapping Accuracy: ${catAccuracy.toFixed(1)}% (${passedCategory}/${totalCases})`,
    );
  ...
  it(`should achieve 100% Bill-Splitting Math Precision`, () => {
    ...
    console.log(
      `📊 Bill-Splitting Math Accuracy: ${splitAccuracy.toFixed(1)}% (${passedSplits}/${totalCases})`,
    );
